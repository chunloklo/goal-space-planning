...
class GoalLearner_QRC_NN():
    def __init__(self, state_shape, num_actions: int, step_size: float, epsilon: float, beta: float = 1.0):
        ...
        _, x = self.value_net.apply(net_params, dummy_state)
    ...
    def _loss(self, params: hk.Params, data):
        ...
        v_loss, h_loss = jax.vmap(partial(qc_loss, self.epsilon), in_axis=0)(q, a, goal_policy_cumulant, goal_discount, qp, h)
        h_loss = h_loss.mean()
        v_loss = v_loss.mean()
    ...
    def get_goal_outputs(self, x: npt.ArrayLike) -> np.ndarray:
        return self._get_goal_output(self.params, x)

    def update(self, data, polyak_stepsize:float=0.005):
        self.params, self.opt_state, delta = self.f_update(self.params, self.target_params, self.opt_state, data)
        return delta
