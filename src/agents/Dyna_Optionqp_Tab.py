...
class Dyna_Optionqp_Tab:
    def __init__(self, features: int, actions: int, params: Dict, seed: int, options):
        ...
        self.tau = np.zeros(((int(features/self.num_actions), self.num_actions+len(self.options))))
        self.a = -1
        self.x = -1

        self.Q = np.zeros((int(self.features/self.num_actions), self.num_actions+len(self.options)))
        self.model = {}
    def FA(self):
        return "Tabular"
