...
class HRL_NN:
    def __init__(self, problem: 'BaseProblem'):
        ...
        self.behaviour_learner = QLearner_NN((4,), self.num_goals, self.step_size)
    ...
    def _OptionValueUpdate(self, sp):
        ...
        if self.buffer.num_in_buffer >= self.min_buffer_size_before_update:
            ...
            # data['target'] = np.empty((OCI_batch_size, self.num_actions + self.num_goals))
            # data['option_mask'] = np.full((OCI_batch_size, self.num_actions + self.num_goals), False)
