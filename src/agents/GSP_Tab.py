...
class GSP_Tab:
    def __init__(self, problem: 'BaseProblem'):
        ...
        self.goal_states = problem.get_goals()
        self.num_goals = len(self.goal_states)

        # Do we want to pre-learn goals?
        self.learn_goal_policies = param_utils.parse_param(params, 'learn_goal_policies', lambda p: isinstance(p, bool))

        if not self.learn_goal_policies:
            self.goal_policy_learners = problem.get_learned_goal_policies()
        else:
            self.goal_policy_learners = [GoalPolicyLearner() for _ in self.goal_states]
    ...
    def _map_goal_to_state_estimates(self, x, a, xp, r, gamma):
        ...
        self.goal_estimate_learner.update(x, a, r, xp, gamma, option_pi_x, 
            self.state_estimate_learner.r[x], self.state_estimate_learner.gamma[x], self.state_estimate_learner.discounted_goal_transition_prob[x], 
    ...
    def _option_constrained_improvement(self, x, xp):
        ...
        goal_transition_prob = self.state_estimate_learner.discounted_goal_transition_prob
...
class GoalEstimates:
    def __init__(self, num_goals, goal_func):
        self.num_goals = num_goals
    ...
    def update(self, x, option_pi_x, r_s, gamma_s, alpha):
...
class GoalValueLearner:
    ...
    def update(self, goal_gamma, reward_goals, alpha):
