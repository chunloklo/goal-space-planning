...
class GSP_NN:
    def __init__(self, problem: 'BaseProblem'):
        ...
        self.behaviour_learner = ActorCritic_NN((4,), 5, self.step_size)
        ...
        self.OCI_update_interval = param_utils.parse_param(params, 'OCI_update_interval', lambda p : isinstance(p, int) and p >= 1) # Number of update steps between each OCI update
    ...
    def _direct_rl_update(self):
        if self.buffer.num_in_buffer >= self.min_buffer_size_before_update:
            ...
            self.behaviour_learner.update(data, polyak_stepsize=0.0005)

    def _state_to_goal_estimate_update(self):
        if self.goal_buffer.num_in_buffer >= self.min_buffer_size_before_update:
            num_batches = 1
            for _ in range(num_batches):
                data = self.goal_buffer.sample(self.batch_size)
                self.goal_learner.update(data, polyak_stepsize=0.0005)
    ...
    def _goal_estimate_update(self, s, a, sp, r, gamma, terminal, goal_init, goal_terms):
        ...
        self.goal_estimate_learner.update(s, goal_policies, goal_r, goal_gammas, 0.1, r, sp, goal_terms, current_val, goal_init)
    ...
    def _OCI(self):
        ...
        if self.buffer.num_in_buffer >= self.min_buffer_size_before_update:
            ...
            self.behaviour_learner.OCI_update(OCI_data, polyak_stepsize=0.0005)
    ...
    def update(self, s: Any, a, sp: Any, r, gamma, terminal: bool = False):
        ...
        if r == 10000:
            self.num_term += 1
            print(f'terminated! num_term: {self.num_term}')
...
class ActorCritic_NN():
    def __init__(self, state_shape, num_actions: int, learning_rate: float):
        ...
        def q_function(states):
            ...
            policy_head = hk.Linear(self.num_actions)
            value_head = hk.Linear(self.num_actions)

            features = mlp(states)

            return jax.nn.softmax(policy_head(features)),  value_head(features)

        self.f_qfunc = hk.without_apply_rng(hk.transform(q_function))
        self.f_opt = optax.adam(learning_rate)

        def get_output(params: hk.Params, x: Any):
            policy, action_values = self.f_qfunc.apply(params, x)
            return policy, action_values

        def OCI_loss(params: hk.Params, data):
            target = data['target']
            x = data['x']
        ...
        def OCI_update(params: hk.Params, target_params: hk.Params, opt_state, data):
            grads = jax.grad(OCI_loss)(params, data)
            updates, opt_state = self.f_opt.update(grads, opt_state)
            params = optax.apply_updates(params, updates)
            return params, opt_state

        self.f_get_q_values = jax.jit(get_output)
    ...
    def update(self, goal_gamma, reward_goals, goal_bonus, goal_init, goal_baseline):
        ...
        for _ in range(num_planning_steps):
            ...
            for g in range(self.num_goals):
                ...
                # if g == 5:
                #     print(f'{goal_baseline[g]} {returns} {valid_goals} {self.goal_values[g]}')
