        ...
        if self.obs_type == 'full_state':
            ...
            if self.cnn_type == 'impala':
                ...
                self.obs_encoder = DanijarEncoder(prefix='encoder', dtype=self.dtype, norm=self.norm, act=self.act,
                                                  cnn_depth=48)
            elif self.cnn_type == 'pinball':
                self.obs_encoder = PinballEncoder(prefix='encoder', dtype=self.dtype, norm=self.norm, act=self.act,
                                                  cnn_depth=48)
    ...
    def setup(self):
        if self.obs_type == 'full_state':
            ...
            if self.cnn_type == 'impala':
                ...
                self.representation = nn.Dense(32 * 48, kernel_init=default_mlp_init(),
                                               dtype=self.dtype)
                self.obs_decoder = PinballDecoder(prefix='decoder',
                                                  last_channel=self.channels * self.framestack,
                                                  norm=self.norm, act=self.act, cnn_depth=48,
                                                  dtype=self.dtype)
...
class PinballEncoder(nn.Module):
    """Original Dreamerv2 cnn architecture."""
    prefix: str
    norm: bool
    act: str
...
class PinballDecoder(nn.Module):
    """Original Dreamerv2 cnn architecture."""
    prefix: str
    norm: bool
    act: str
