...
from src.utils import analysis_utils, numpy_utils
...
def generatePlot(json_handle):
    ...
    print(data.keys())
    skipped_state = data["skipped_states"]
    skipped_state = skipped_state[:, 0, :, :]
    skipped_state = np.mean(skipped_state, axis=0)
    print(skipped_state.shape)

    state_visitation = data["state_visitation"]
    state_visitation = state_visitation[:, 0, :, :]
    state_visitation = np.mean(state_visitation, axis=0)
    ...
    anim_file_name = f'{experiment_name}_skipped_states.mp4'
    ...
    hasOptions = skipped_state.shape[-1] > 4
    hasOptions = False

    num_options = skipped_state.shape[-1] - 4
    ...
    def draw_func(i):
        ...
        state_visit_count = np.sum(state_visitation[np.max([start_frame, i - interval]) : i, :], axis=0)
        skip_count = np.sum(skipped_state[np.max([start_frame, i - interval]) : i, :], axis=0)
        ...
        for r in range(ROW_MAX):
            for c in range(COLUMN_MAX):
                skip = skip_count[r * COLUMN_MAX + c]
                state_count = state_visit_count[r * COLUMN_MAX + c]

                
                q_value = [skip / state_count if state_count != 0 else 0] * 4
                
                # q_value = [np.sum(q_value)] * 4
