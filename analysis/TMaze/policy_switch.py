...
def generatePlot(json_handle):
    ...
    anim_file_name = f'{experiment_name}_policy_switch.mp4'
    ...
    def draw_func(i):
        ...
        # q_values = data[i, :] - data[np.max([start_frame, i - interval]), :]
        q_values = data[i, :] 
        # print(q_values)
        ...
        for r in range(ROW_MAX):
            for c in range(COLUMN_MAX):
                ...
                # print(q_value)
                # q_value -= np.min(q_value)
                # if np.linalg.norm(q_value, ord=1) == 0:
                #     q_value = [0] * 4
                # else:
                    # q_value /= np.linalg.norm(q_value, ord=1)
