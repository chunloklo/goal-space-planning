...
def generatePlot(json_handle):
    ...
    # Getting file name
    save_file = prompt_user_for_file_name('./visualizations/', 'state_estimate_goal_prob_', '', 'mov', timestamp=True)
    ...
    num_goals = len(env.get_options()[0].termination_set)
    ...
    for ax in ax_list:
        _texts, _patches = _plot_init(ax, columns = env.size, rows = env.size)
        texts.append(_texts)
        ...
        for r in range(env.size):
            for c in range(env.size):
                try:
                    ...
                    for g in range(6):
                        for a in range(4):
                            scaled_value = scale_value(state_data[a, g], min_val, max_val, post_process_func=lambda x: x)
                            patches[g][r][c][a].set_facecolor(colormap(scaled_value))
                            # colors = ["red", "green", "blue", "orange"]
                            # patches[i][j][a].set_facecolor(colors[a])
                            texts[g][r][c][a].set_text(round(state_data[a, g], 2))
...
if __name__ == "__main__":
    ...
    data = load_data(parameter_list[0], 'state_estimate_goal_prob')
