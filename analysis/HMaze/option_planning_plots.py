...
if __name__ == "__main__":
    ...
    data = load_data_from_config_zodb(parameter_list[0])
    print(data.keys())

    NUM_GOALS = 7
    NUM_OPTIONS = 2

    key_data = np.array(data['goal_values'])
    # print(key_data)
    # print(key_data[0, -1, 5])
    print(key_data.shape)
    plt.plot(key_data[0], label=range(NUM_GOALS), alpha=0.7)
    ...
    key_data = np.array(data['goal_r'])
    print(key_data.shape)

    plt.figure()
    for o in range(NUM_OPTIONS):    
        plt.subplot(1, 2, o + 1)
        # print(key_data[0], :, :, o].shape)
        # sdf
        plt.plot(key_data[0, :, :, o], label=range(NUM_GOALS), alpha=0.7)
        plt.legend()
        plt.title(f'option: {o}')

    save_file = get_file_name('./plots/', f'goal_r', 'png', timestamp=True)
    plt.savefig(f'{save_file}', dpi = 300)

        
    key_data = np.array(data['goal_gamma'])
    ...
    save_file = get_file_name('./plots/', f'goal_gamma', 'png', timestamp=True)
    plt.savefig(f'{save_file}', dpi = 300)

    key_data = np.array(data['goal_transition'])
    print(key_data.shape)

    scale = 4
    fig = plt.figure(figsize=(7 * scale, 2 * scale))
    for o in range(NUM_OPTIONS):
        for g in range(NUM_GOALS):
            index = o * NUM_GOALS + g + 1 
            ax = fig.add_subplot(2, 7, index)
            ax.plot(key_data[0, :, g, o], label=range(NUM_GOALS))
            ax.set_title(f'option {o} goal {g}')
    plt.legend()

    save_file = get_file_name('./plots/', f'goal_transition', 'png', timestamp=True)
    plt.savefig(f'{save_file}', dpi = 300)
