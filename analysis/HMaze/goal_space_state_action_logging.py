...
def generatePlot(data):
    ...
    patches = []
    for ax in axes:
        _texts, _patches = _plot_init(ax, columns = COLUMN_MAX, rows = ROW_MAX, center_arrows=False)
        texts.append(_texts)
        patches.append(_patches)
    

    min_val = np.min(data[start_frame:max_frame])
    ...
    def draw_func(i):
        ...
        for r in range(ROW_MAX):
            for c in range(COLUMN_MAX):
                if (r, c) not in env.valid_grids:
                    continue
                x = tab_feature.encode((r, c))
                for g in range(num_goals):
                    q_value = q_values[x, :, g]
                    # print(q_value)
                    for a in range(4):
                        ...
                        texts[g][r][c][a].set_text(round(q_value[a] * NUM_FACTOR, 2))
...
if __name__ == "__main__":
    ...
    data = load_data(parameter_list[0], 'state_r')
    print(data.shape)
